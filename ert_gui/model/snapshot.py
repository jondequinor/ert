from ert_shared.ensemble_evaluator.entity.snapshot import PartialSnapshot, SnapshotDict
from ert_gui.model.dev_helpers import (
...
class SnapshotModel(QAbstractItemModel):
    ...
    def _add_partial_snapshot(self, partial: PartialSnapshot, iter_: int):
        ...
        for real_id, real in partial_d["reals"].items():
            ...
            real_index_bottom_right = self.index(real_node.row(), self.columnCount(iter_index) - 1, iter_index)
            ...
            for stage_id, stage in real["stages"].items():
                ...
                stage_index_bottom_right = self.index(stage_node.row(), self.columnCount(real_index) - 1, real_index)
                ...
                for step_id, step in stage["steps"].items():
                    ...
                    step_index_bottom_right = self.index(step_node.row(), self.columnCount(stage_index) - 1, stage_index)
                    ...
                    for job_id, job in step["jobs"].items():
                        ...
                        if job.get("data", {}).get("max_memory_usage"):
                            job_node.data["data"]["max_memory_usage"] = job["data"]["max_memory_usage"]

                        job_index = self.index(job_node.row(), 0, step_index)
                        job_index_bottom_right = self.index(job_node.row(), self.columnCount() - 1, step_index)
    ...
    def _add_snapshot(self, snapshot: SnapshotDict, iter_: int):
    ...
    def parent(self, index):
        ...
        if not hasattr(child_item, "parent"):
            print("index pointed to parent-less item", child_item, index.row(), index.column(), index.parent(), index.parent().isValid())
            return QModelIndex()
